...
public class GeneralQuery18 {
	...
	public static class SpanishEconomicalGrant implements SparkTask<Double> {
		...
		public Iterable<KeyCountPair<Double>> computeSparkTask(String[] inputFiles) {
			...
			try {
				...
				Dataset<Row> studentsGrant = students
						.filter(col("countryOfHomeInstitution").equalTo(SPAIN))
				...
				economicalGrant.add(new KeyCountPair<>(SPAIN, grant));
			} catch (Exception e) {
	...
	// spark-submit --class systematicchaos.studentmobility.GeneralQuery18 --master local[*] target/spark-1.0.0.jar ../data/Student_Mobility.csv [out/spark18-output]
	public static void main(String[] args) throws Exception {
		SparkJobLauncher.launchSparkJob(new SpanishEconomicalGrant(), args);
	}
}
